...
- **Web dashboard.** The first workflow run lazily starts a local HTTP+SSE server and serves a live browser dashboard — the run forest plus every sub-agent's transcript — from assets embedded in the binary. Set `AUK_NO_DASHBOARD=1` to opt out.
