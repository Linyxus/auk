...
/** Display width of Unicode text in terminal columns.
  ...
  *
  * The tables are generated from Unicode 16.0 data and match what modern
  * terminals (Ghostty, kitty, WezTerm, iTerm2, foot) compute per codepoint:
  *
  ...
  *     modifiers U+1F3FB–1F3FF (see below).
  *   - wide: East Asian Width `W` or `F`. Since Unicode 9 this includes every
  *     default-emoji-presentation character (✅ 🚀 🟢 …), so emoji width comes
  *     out of the same table. Ambiguous-width characters count 1 (the
  *     universal terminal default).
  *
  * Any drift between these tables and the terminal's own width opinion makes
  * the renderer's relative cursor math land off-target and smears the screen,
  * so precision here is load-bearing, not cosmetic.
  *
  ...
  *
  * Regenerate by sweeping codepoints with the rule above (Python's
...
object Width:
  ...
  // East Asian Width W/F — CJK, Hangul, fullwidth forms, and all emoji with
  // default emoji presentation (Unicode aligned EAW with Emoji_Presentation).
