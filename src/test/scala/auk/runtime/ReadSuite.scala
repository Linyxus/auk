...
class ReadSuite extends munit.FunSuite:
  ...
  test("an empty file reports it is empty and points to write"):
    given dir: Path = tempDir()
    write(dir, "f.txt", "")
    val r = run("f.txt")
    assertEquals(r.isError, false)
  ...
  test("a missing file is reported (not an error) and points to write"):
    given dir: Path = tempDir()
    val r = run("nope.txt")
  ...
  test("the missing-file message echoes the path as given"):
    given dir: Path = tempDir()
    val r = run("ghost.txt")
    assertEquals(r.isError, false, r.output)
