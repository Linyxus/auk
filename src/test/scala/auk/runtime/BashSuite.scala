...
class BashSuite extends munit.FunSuite:
  ...
  private def tempDir(): String = TestFs.tempDir("auk-bash")
  ...
  asyncTest("captures stdout and reports success"):
  ...
  asyncTest("merges stderr into the output"):
    val r = run("echo out; echo err 1>&2")
    assert(r.output.contains("out"), r.output)
    assert(r.output.contains("err"), r.output)
    assertEquals(r.isError, false)

  asyncTest("preserves multi-line output verbatim, including the trailing newline"):
    val r = run("printf 'a\\nb\\nc\\n'")
    assertEquals(r.output, "a\nb\nc\n")

  asyncTest("empty output on success reads as (no output)"):
    val r = run("true")
  ...
  asyncTest("non-zero exit is an error and records the code"):
    val r = run("exit 3")
    assert(r.isError)
    assertEquals(r.metadata("exitCode"), "3")
    assert(r.output.contains("[command exited with code 3]"), r.output)

  asyncTest("unknown command surfaces shell error and non-zero exit"):
  ...
  asyncTest("runs in the context working directory"):
    val dir = tempDir()
    TestFs.write(TestFs.join(dir, "marker.txt"), "in-cwd")
  ...
  asyncTest("stdin is closed so readers see EOF instead of hanging"):
  ...
  asyncTest("a command exceeding the timeout is killed and flagged"):
  ...
  asyncTest("output past the cap is truncated and flagged"):
  ...
  asyncTest("a denied command does not run and reports an error"):
    val dir = tempDir()
    val r = run("touch should_not_exist.txt", ctx = ctxAt(dir, ApprovalPolicy.DenyAll))
    assert(r.isError)
    assert(r.output.contains("not approved"), r.output)
    assert(
      !TestFs.exists(TestFs.join(dir, "should_not_exist.txt")),
      "denied command must not have executed"
    )

  asyncTest("approval policy receives the tool name and command"):
  ...
  asyncTest("empty command is rejected without running a process"):
    val r = run("   ")
    assert(r.isError)
  ...
  asyncTest("dispatches through the ToolRegistry"):
    locally:
  ...
  asyncTest("registry advertises a bash schema with a required command"):
  ...
  asyncTest("exit code 1 is an error with the matching footer"):
    val r = run("false")
    assert(r.isError)
  ...
  asyncTest("exit code 127 reported for not-found, exit code 126 for non-executable"):
    val r = run("command_that_surely_does_not_exist_42")
    assert(r.isError)
    assertEquals(r.metadata("exitCode"), "127")

  asyncTest("the shell wraps exit codes into the 0..255 range"):
  ...
  asyncTest("a fatal signal yields a 128+signal exit code"):
  ...
  asyncTest("non-zero exit still carries the command's own output before the footer"):
  ...
  asyncTest("stderr-only output on a failing command is captured"):
    val r = run("echo boom 1>&2; exit 1")
    assert(r.output.contains("boom"), r.output)
    assert(r.isError)
    assertEquals(r.metadata("exitCode"), "1")

  asyncTest("output without a trailing newline is preserved exactly"):
    val r = run("printf 'no-newline'")
    assertEquals(r.output, "no-newline")
    assertEquals(r.isError, false)

  asyncTest("a lone leading newline from the command is stripped to empty"):
  ...
  asyncTest("interleaved stdout and stderr both appear in order-independent output"):
  ...
  asyncTest("non-ASCII UTF-8 bytes round-trip through the capture"):
    val r = run("printf 'caf\\xc3\\xa9 \\xe2\\x9c\\x93'")
    assertEquals(r.output, "café ✓")
    assertEquals(r.isError, false)

  asyncTest("a successful command with no output reports (no output) and metadata"):
    val r = run(":")
    assertEquals(r.output, "(no output)")
    assertEquals(r.isError, false)
    assertEquals(r.metadata("exitCode"), "0")
  ...
  asyncTest("multi-statement script with && chains and a final newline"):
    val r = run("printf 'first\\n' && printf 'second\\n'")
    assertEquals(r.output, "first\nsecond\n")
    assertEquals(r.isError, false)

  asyncTest("a newline-separated multiline script runs as one command"):
    val r = run("x=1\ny=2\necho $((x + y))")
    assertEquals(r.output.trim, "3")
    assertEquals(r.isError, false)

  asyncTest("pipes are evaluated by the shell"):
    val r = run("printf 'a\\nb\\nc\\n' | wc -l | tr -d ' '")
    assertEquals(r.output.trim, "3")
    assertEquals(r.isError, false)

  asyncTest("redirects write files relative to the working directory"):
    val dir = tempDir()
    val r = run("echo redirected > out.txt", ctx = ctxAt(dir))
    assertEquals(r.isError, false)
    assert(TestFs.exists(TestFs.join(dir, "out.txt")), "redirect should create the file")
    assertEquals(TestFs.read(TestFs.join(dir, "out.txt")).trim, "redirected")

  asyncTest("pipefail semantics are NOT enabled by default: tail of a pipe wins"):
    // Without `set -o pipefail`, the exit status is that of the last stage.
    val r = run("false | true")
    assertEquals(r.metadata("exitCode"), "0")
    assertEquals(r.isError, false)

  asyncTest("environment variables visible to the shell are expanded"):
    val r = run("echo $HOME")
  ...
  asyncTest("pwd reflects the context working directory"):
    val dir = tempDir()
    val r = run("pwd", ctx = ctxAt(dir))
    // macOS temp dirs live under symlinked /var -> /private/var, so the reported
    // path may carry a different prefix than `dir`. The unique temp dir name is
    // the part that must match, so compare basenames.
    assertEquals(PathOps.basename(r.output.trim), PathOps.basename(dir))

  asyncTest("each run gets its own fresh working directory by default"):
  ...
  asyncTest("a timeout below 1ms is floored to 1ms, not zero or negative"):
  ...
  asyncTest("an explicit large timeout does not delay a fast command"):
    val start = System.nanoTime()
  ...
  asyncTest("timeout returns whatever output was produced before the kill"):
  ...
  asyncTest("timeout footer takes precedence over the exit-code footer"):
  ...
  asyncTest("a denied command yields no exit-code metadata"):
    val r = run("echo nope", ctx = ctxAt(tempDir(), ApprovalPolicy.DenyAll))
    assert(r.isError)
    assert(r.output.contains("not approved"), r.output)
    assertEquals(r.metadata, Map.empty[String, String])

  asyncTest("approval is consulted before an empty command is even rejected? no -- empty wins"):
  ...
  asyncTest("the approval summary carries the full multiline command text"):
    var seen: Option[ApprovalRequest] = None
    val recording = new ApprovalPolicy:
      def request(req: ApprovalRequest)(using Async): Boolean =
        seen = Some(req); true
  ...
  asyncTest("registry dispatch surfaces a non-zero exit as an error"):
    locally:
      given RuntimeContext = ctxAt(tempDir())
      val registry = ToolRegistry.of(Bash)
      val out = registry.dispatch(
  ...
  asyncTest("registry dispatch honours an explicit timeoutMs argument"):
    locally:
      given RuntimeContext = ctxAt(tempDir())
      val registry = ToolRegistry.of(Bash)
      val out = registry.dispatch(
